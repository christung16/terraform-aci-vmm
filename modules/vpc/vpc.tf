...
data "aci_lacp_policy" "lacp" {
  name = var.port_channel_status
}

resource "aci_leaf_access_bundle_policy_group" "leaf_access_bundle_pg" {
  name = var.name
  lag_t = var.lag_t
  ...
  relation_infra_rs_lacp_pol = data.aci_lacp_policy.lacp.id
}
...
resource "aci_access_port_selector" "access_port_selector" {
  ...
  relation_infra_rs_acc_base_grp = aci_leaf_access_bundle_policy_group.leaf_access_bundle_pg.id
}
